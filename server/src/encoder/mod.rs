...
pub fn default_encoder(
    ...
    Ok(if cfg!(feature = "nvidia-hwenc") {
        info!("using nvidia encoder");
        Box::new(nvidia::LVNvidiaEncoder::new(width, height, bitrate, fps)?)
    } else {
        Box::new(LVOpenH264Encoder::new(width, height, bitrate, fps)?)
    })
}
