...
impl LVVideoDecoder for LVVAAPIDecoder {
    ...
    fn decode(&mut self, timestamp: u64, packet: &[u8]) -> Result<(), Box<dyn std::error::Error>> {
        ...
        loop {
            ...
            ret_val = match self.decoder.decode(timestamp, packet) {
                Ok(amt_decoded) => {
                ...
                Err(e) => {
                    error!("couldn't decode for reason {e:?}");
                    panic!();
                    Err(Box::new(e))
                }
            };

            info!("must retry decode {must_retry_decode}");
            if !must_retry_decode {
                break;
            } else {
                println!("retrying decode because there were events")
            }
        }
